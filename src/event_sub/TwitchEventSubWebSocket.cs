...
public sealed class TwitchEventSubWebSocket {
    ...
    private async Task ReceiveLoopAsync(ClientWebSocket socket, CancellationToken token) {
        ...
        while (socket.State == WebSocketState.Open && !token.IsCancellationRequested) {
            ...
            if (await Task.WhenAny(receiveTask, Task.Delay(_keepAliveTimeout, token)) != receiveTask)
                return;
    ...
    private async Task SubscribeAllInternal() {
        ...
        foreach (var sub in _subscriptions) {
            try {
                var errorCallback = _subscribed
                                        ? null
                                        : OnWebSocketError;
                await sub.SubscribeAsync(SessionId, _credentials, errorCallback);
            }
            catch (Exception ex) {
                OnWebSocketError?.Invoke(this, ex.Message);
            }
